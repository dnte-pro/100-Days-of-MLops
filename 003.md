...
### 1. Change Directory
In the terminal change the directory
```bash
cd /root/code/fraud-detection
```
In the IDE, open the requirements.in.

### 2. Inspect the file ####requirements.in
Check the details:
```bash
...
- mlflow>=100.0 is invalid/unresolvable
- pandas is missing
- every package must have a valid version constraint

### 3.Change the requirements.in
In the IDE, change the contents in requirements.in
...
### 4. Compile the corrected specification into a pinned lockfile
```bash
uv pip compile requirements.in -o requirements.txt
```
The resulting requirements.txt must pin each of the four top-level packages to an exact version using ==,
and must also include the transitive dependencies that uv resolved.
...
### 5. Confirming packages
Confirm the packages are pinned:
```bash
